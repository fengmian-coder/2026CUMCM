...
p(f"主方案使用00:00、06:00、12:00预报，在2025年2月1日至12月31日自然日窗口内，总费用为{main['natural_totals']['total_cost_yuan']}元。相对仅使用00:00预报的第三问对照，费用下降127285.03930760734元。加入18:00更新并未进一步降低本次全年费用，不能将预报次数越多视为必然更优。")
h('一 时间轴与决策边界')
p('沿用第二问已确认的建模假设：每天00:00制定的144段计划从当天00:10开始，至次日00:10结束。每个整点发布的新信息只影响下一个10分钟边界以后的动作；这属于与模板协调的实施假设，并非原题明示的通信延迟。')
table(['发布时间','允许重新优化的序号','对应区间'],[['00:00','1—144','00:10—次日00:10'],['06:00','37—144','06:10—次日00:10'],['12:00','73—144','12:10—次日00:10'],['18:00 对照','109—144','18:10—次日00:10']],[1.1,1.9,3.7])
p('主方案只在06:00和12:00允许调整；18:00仅在对照实验启用。每次重算全部剩余时段，而非只重算接下来的6小时。此前已执行及当前整点到下一10分钟的动作被冻结。下一轮以当前有效方案继续递推，因此既不会回滚已执行动作，也无需在6小时边界强制回到原SOC。')
...
p('主方案在12:00最后一次更新后，次日00:00—00:10也保持锁定；次日零点报告前143段执行后的电量，次日新优化使用再执行末段后的00:10电量。实际源荷只用于事后紧急购电和剩余量结算，不用于提前挑选当天动作。')
...
table(['预报更新组合','自然日总费用 元'],[[label,cases[k]['natural_totals']['total_cost_yuan']] for k,label in [('only_0','仅00:00'),('updates_06','00:00及06:00'),('updates_12','00:00及12:00'),('updates_18','00:00及18:00'),('updates_0612','00:00及06:00及12:00 主方案'),('updates_0618','00:00及06:00及18:00'),('updates_1218','00:00及12:00及18:00'),('updates_061218','四时点全部使用')]], [2.7,3.9])
p('主方案相对只用00:00节省127285.03930760734元。在已加入06:00和12:00的基础上再加入18:00，总费用增加2259.214770646766元。本次18:00更新没有体现进一步省钱的价值，但这一结果是样本期回测结论，不能当作未来所有日期的保证。')
p('主方案06:00和12:00均触发334次调整，18:00不调整。四时点对照在18:00触发325次调整。触发次数反映当时预计收益超过阈值，不等于实际收益次数。预测分布与真实值有偏差，跨日储电量也会影响后续成本，因此预期改善不保证事后费用下降。')
p('已枚举全部8种时点组合，经比较后采用00:00、06:00、12:00方案填写result3。分别从四时点删除06:00、12:00使费用增加73818.4469681494元、86309.21754294634元；删除18:00节省2259.214770646766元。该选择基于样本内回测，不是独立样本最优保证。')

page();h('七 对照结果图与敏感性')
picture('q3_update_cost_comparison.png',5.8)
...
p('上述窗口和阈值试验均使用主方案的00:00、06:00、12:00，且每组独立运行全年。主参数28日、0.1%在运行前确定；报告如实呈现其他参数结果，不把完整回测期同时作为参数选择后的独立测试集。')
p(f"不退费口径总费用为{cases['no_refund']['natural_totals']['total_cost_yuan']}元，紧急购电量为{cases['no_refund']['natural_totals']['emergency_kwh']} kWh。退费假设应结合总费用与紧急购电量评价。旧四时点敏感性结果单独归档。")

page();h('八 预测结果与结果表填写')
p('下图与来源计数覆盖四个可用发布时间，用于信息价值分析；主方案不执行18:00更新。')
picture('q3_forecast_mae.png',6.0)
...
p(f'充放电页按自然日六个4小时区间保存最终动作，零点及24:00电量取正确边界。紧急购电页按自然日合并连续正缺口区间，无紧急购电日期不另列零记录。共334个计划日、2004行充放电汇总和{audit["emergency_records"]}段紧急购电记录。')
p(f'计划表行合计覆盖2月1日00:10至次年1月1日00:10；自然日统计覆盖2月1日00:00至次年1月1日00:00。两者窗口不同，不混加。主方案计划窗口总费用为{main["plan_totals"]["total_cost_yuan"]}元。')
...
p('outputs/q3/main保存主方案逐时执行表execution.csv、计划与最终动作数组schedules.npz、自然日报告数组natural.npz以及每次调整的keep/adjust比较adjustment_events.csv。outputs/q3/comparison.json保存主方案、全部时点组合与敏感性试验汇总。forecast_selection_w28.csv记录每天各时点的权重、来源与历史评分。')
...
p('第三问相对第二问总费用下降322297.8430737294元，同时改变了零点光伏输入和日内更新机制，不能将全部差额归因于日内预报。单独评价日内更新，应采用第三问仅00:00方案作为对照。')
