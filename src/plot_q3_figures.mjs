...
const root=path.resolve(import.meta.dirname,'..'),out=path.join(root,'figures');
const C={ink:'#263746',blue:'#466F87',cyan:'#78B6C8',yellow:'#F2D98E',coral:'#E48578',green:'#98AF1E'};
const all=JSON.parse(fs.readFileSync(path.join(root,'outputs/q3/comparison.json'),'utf8'));
...
{
  const keys=['only_0','updates_06','updates_0612','main'],labels=['仅00:00','增加06:00','增加06:00与12:00','四时点全部使用'];
  const v=keys.map(k=>all.find(x=>x.case===k).natural_totals.total_cost_yuan/10000);
  const x=a=>430+(a-1435)/20*970;let b='<text x="800" y="70" text-anchor="middle" style="font-size:36px;font-weight:bold">不同预报时点组合的实际总费用</text>';
  for(let a=1435;a<=1455;a+=5)b+=`<line x1="${x(a)}" y1="150" x2="${x(a)}" y2="730" class="grid"/><text x="${x(a)}" y="780" text-anchor="middle">${a}</text>`;
  v.forEach((a,i)=>{const y=215+i*145,c=i===2?C.green:i===3?C.coral:C.blue;b+=`<text x="390" y="${y+8}" text-anchor="end">${labels[i]}</text><line x1="430" y1="${y}" x2="${x(a)}" y2="${y}" stroke="${c}" stroke-width="5"/><circle cx="${x(a)}" cy="${y}" r="12" fill="${c}"/><text x="${x(a)+18}" y="${y-22}">${a.toFixed(4)}</text>`;});
