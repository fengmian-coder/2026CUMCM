...
def evaluate(solution,p,load,pv,base=None,refund=True):
    ...
    cash=grid_cost(solution['purchase'],p,base,refund)+float(losses.mean())
...
def solve(p,e0,load,pv,base=None,refund=True):
    ...
    obj=np.zeros(n);obj[g]=p;obj[b.ravel()]=np.tile(5*p/S,S);obj[e[-1]]=-VALUE;obj[z]=LAMBDA;obj[v]=LAMBDA/((1-ALPHA)*S)
    constant=0.
    if base is not None:
    ...
    add(np.repeat(S*T+np.arange(S),T),b.ravel(),np.tile(5*p,S));add(S*T+np.arange(S),np.full(S,z),-1);add(S*T+np.arange(S),v,-1)
    ...
    ev=evaluate(result,p,load,pv,base,refund)
